...
interface AddButtonProps {
    ...
    variant: 'default' | 'outline' | 'link' | 'ghost' | 'destructive' | undefined;
    permission?: string;
}
...
interface ButtonProp {
    ...
    type: 'button' | 'submit' | 'reset' | undefined;
    label: string;
    variant: 'default' | 'outline' | 'ghost' | 'link' | 'destrctive' | undefined;
    className: string;
}
...
export const CustomModalForm = ({
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange} modal>
            ...
            <DialogContent onInteractOutside={(e) => e.preventDefault()} className="sm:max-w-[800px] dark:bg-gray-800 dark:text-gray-400">
                <DialogHeader>
                        ...
                        {fields.map((field) => (
                            <div key={field.key} className="grid gap-2">
                                ...
                                {field.type === 'textarea' ? (
                                    ...
                                    <div className="space-y-2">
                                        {extraData &&
                                            Object.entries(extraData).map(([module, permission]) => (
                                                <div key={module} className="mb-4 border-b pb-5">
                                                    <h4 className="text-grey-700 text-sm font-bold capitalize">{module}</h4>
                                                    <div className="ms-4 mt-2 grid grid-cols-3 gap-2">
                                                        {permission.map((permission) => (
                                                            <label key={permission.id} className="text-md flex items-center gap-2">
                                                                <input
                                                                    type="checkbox"
                                                                    name={field.name}
                                                                    disabled={processing || mode === 'view'}
